...
describe('ProductComponent', () => {
  ...
  it('should return correct error message for minlength field', () => {
    ...
    expect(component.getMessageError('test')).toEqual('Este campo debe tener mínimo 3 caracteres');
  });
  ...
  it('should set error message on failure', () => {
    (productService.addProduct as jest.Mock).mockReturnValue(throwError(() => new Error('error')));
    component.pushProduct();
  ...
  it('should update product if id exists', () => {
    ...
    component.onSave();
    expect(productService.updateProduct).toHaveBeenCalledWith('1', { test: 'valid value' });
    expect(router.navigate).toHaveBeenCalledWith(['/']);
  });
